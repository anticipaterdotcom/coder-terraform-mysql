...
resource "docker_container" "workspace" {
  ...
  entrypoint = var.is_local ? ["/bin/bash", "-c", "tail -f /dev/null"] : ["sudo su -l www-data -s /bin/bash", "-c", replace(coder_agent.shopware.init_script, "/localhost|127\\.0\\.0\\.1/", "host.docker.internal")]
