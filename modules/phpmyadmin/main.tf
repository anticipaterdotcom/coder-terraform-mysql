...
resource "docker_container" "phpmyadmin" {
  ...
  entrypoint = var.is_local ? ["/bin/bash", "-c", "tail -f /dev/null"] : ["sh", "-c", replace(coder_agent.phpmyadmin.init_script, "/dev\\.anticipater\\.local|localhost|127\\.0\\.0\\.1/", "host.docker.internal")]
  env = [
